...
if(home_dir){
  ...
  inputs = 'dataset=aian:models=acs,pep,wp:n.sample=100:burnin=50:outcome=census:family=negbin:use_softmax=T:fixed_rho=-1:fixed_tau2=-1:sigma2_prior_shape=50:sigma2_prior_rate=0.5:tau2_prior_shape=1:tau2_prior_rate=1:theta_prior_shape=0.001:theta_prior_rate=0.001:stan_path=code/CAR_leroux_sparse_negbin_alpha_FE.stan:CV_blocks=NULL:return_quantiles=F:output_path_addition=softmax_interceptonly:chains_cores=1:alpha_variance_prior=-1:preprocess_scale=F:fixed_effects=pep_fulldensity_proportion'
}else{
  # cluster inputs
