...
inputs = c('R=2:dataset=NY:N_models=3:n.sample=20:burnin=10:family=negbin:use_softmax=T:variances=100,100,100:means=100,100,100:rho=0.99:fixed_rho=-1:tau2=1:fixed_tau2=-1:tau2_prior_shape=1:tau2_prior_rate=1:theta_true=100:theta_gamma_prior=0:num_y_samples=3:stan_path=code/CAR_leroux_sparse_negbin_NC.stan:CV_blocks=-1:return_quantiles=F:parallel=T:output_path=simulation_noncentered_rho_099_theta_100_softmax_negbin_3models_CV10_ID609610_2025_04_09:chains_cores=1:alpha_variance_prior=1e-04:phi_noncentered=1','1')
...
for(str in strsplit(inputs[[1]],':')[[1]]){
  ...
  if(nn %in% c('R', 'n.sample', 'burnin', 'rho', 'fixed_rho', 'tau2', 'fixed_tau2', 'sigma2', 'sigma2_prior_shape', 'sigma2_prior_rate', 'theta_true', 'tau2_prior_shape', 'tau2_prior_rate', 'num_y_samples', 'CV_blocks','theta_prior_shape','theta_prior_rate', 'theta_gamma_prior','phi_noncentered')){
    val = as.numeric(val)
  }else if(nn == 'N_models'){
