...
# @title Visualization: make_color_norm
...
def run_mcmc_CAR2(init_state: Optional[List[tf.Tensor]] = None,
             target_log_prob_fn: Optional[Callable[..., tf.Tensor]] = None, 
  ...
  # Perform MCMC.
  chain_samples, sampler_stat = run_chain_CAR2(
      init_state=init_state, 
      step_size=step_size,
      target_log_prob_fn=target_log_prob_fn,
      num_steps=num_steps, 
...
@tf.function(experimental_compile=True)
def run_chain_CAR2(init_state: List[tf.Tensor], 
              step_size: float, 
              target_log_prob_fn: Callable[..., tf.Tensor], 
              num_steps: int = 500, 
              burnin: int = 100, 
