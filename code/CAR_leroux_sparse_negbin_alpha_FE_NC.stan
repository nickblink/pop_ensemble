...
functions {
  ...
  */
  ...
  int<lower=0, upper=1> phi_noncentered; // 0 = phi estimated centered. 1 = phi estimated non-centered.
...
parameters {
  ...
  matrix[N, M_phi] phi_estimated; // CAR parameter: number of observations x number of models.
  matrix[num_vars, M_phi] beta; // fixed effects parameters.
}
transformed parameters {
  // variable declarations
  matrix[N, M_phi] phi; // CAR parameter: number of observations by number of models.
  ...
  if(phi_noncentered == 1){
    for(m in 1:M_phi){
	  phi[1:N,m] = sqrt(tau2[m])*phi_estimated[1:N,m];
	}
  }else{
    phi = phi_estimated;
  }
...
model {
  ...
  for(m in 1:M_phi){
    if(phi_noncentered == 1){
	  phi_estimated[1:N, m] ~ sparse_car(1, rho[m], W_sparse, D_sparse, log_detQ[m], N, W_n);
	}else{
	  phi_estimated[1:N, m] ~ sparse_car(tau2[m], rho[m], W_sparse, D_sparse, log_detQ[m], N, W_n);
	}
  }
}
